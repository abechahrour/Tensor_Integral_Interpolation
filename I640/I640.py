...
def get_data(data_path , C_str_feat, C_str, size_str, ext_str, idx, idx_y):
    ...
    y = np.genfromtxt(data_path + "{}_labels_{}{}.txt".format(C_str, size_str, ext_str), delimiter = ',', max_rows = n_train)
    print("Data Loaded")
...
def get_test_data(data_path , C_str_feat, C_str, size_str, ext_str, idx, idx_y):
    ...
    y = np.genfromtxt(data_path + "{}_labels_{}{}.txt".format(C_str, size_str, ext_str), delimiter = ',', max_rows = n_test)
    x= x[:,2*idx:2*idx+2]
    idcs = np.where(np.logical_and(np.abs(y[:,idx_y]) < 200, np.abs(y[:,idx_y]) > 1e-8))
    #idcs = np.where(np.logical_and(x[:,0] < 0, x[:,1] < 0.5))
    x = x[idcs]
...
def init_model(ndims, nodes, layers, activation, loss):
...
def forward_pass(model, x_test, scaler):
    ...
    y_pred = scaler.inverse_transform(np.reshape(y_pred,(-1,1))).squeeze()
    return y_pred
...
def execute(data_path, C_str_feat, C_str, size_str, size_str_test, ext_str ,norm, loss, idx, idx_y,
    ...
    #x_test, y_test = get_test_data(path + 'I640' +'_data/', 'I640','C010mimi0', size_str_test,ext_str_test, idx)
    ...
    if load:
        ...
        model = init_model(ndims, nodes, layers, activation, loss)
