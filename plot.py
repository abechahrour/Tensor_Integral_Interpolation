...
def plot_err(path, NAME, errRe, mean_err, median_err, eta_1, eta_5, eta_10):
    ...
    ax = plt.gca()
    ax.text(0.35, 0.90, r'$\epsilon$ = {:0.3f} %'.format(mean_err),
        verticalalignment='bottom', horizontalalignment='right',
        transform=ax.transAxes,
        color='green', fontsize=15)
    ax.text(0.35, 0.80, r'$\eta1$ = {:0.3f} %'.format(eta_1),
        verticalalignment='bottom', horizontalalignment='right',
        transform=ax.transAxes,
        color='green', fontsize=15)
    ax.text(0.35, 0.70, r'$\eta5$ = {:0.3f} %'.format(eta_5),
        verticalalignment='bottom', horizontalalignment='right',
        transform=ax.transAxes,
        color='green', fontsize=15)
    ax.text(0.35, 0.60, r'$\eta10$ = {:0.3f} %'.format(eta_10),
        verticalalignment='bottom', horizontalalignment='right',
        transform=ax.transAxes,
        color='green', fontsize=15)
...
def plot_loss(path, NAME, history):
...
"""
def plot_grid(path, C_str, NAME, vals, x_test, title, xlabel, ylabel, lead_str, end_str = "", norm = 'symlog'):
    num = int(np.ceil(np.sqrt(vals.shape[1])))
    fig, axs = plt.subplots(num, num, figsize=(14, 8), sharex = True, sharey = True)
    fig.suptitle('Relative Error of Tensor Components' + '\n' + NAME)
    count = 0
    axs = axs.flatten()
    print(axs.shape)
    for i in range(vals.shape[1]):
        ...
        H, xedges, yedges, binnumber = binned_statistic_2d(x_test[:N,0], x_test[:N,1], values = vals[:N, count], statistic=np.mean , bins = [bins, bins])
        ...
        im = axs[i].pcolormesh(XX,YY,H.T, norm= norm, cmap='plasma')
    ...
    for i in range(num**2 - vals.shape[1]):
        fig.delaxes(axs[vals.shape[1]+i])
    fig.savefig(path + "plots/preds/{}_{}_{}.png".format(lead_str, NAME, end_str), bbox_inches="tight")

def plot_2D(path, C_str, NAME, vals, x_test, title, xlabel, ylabel, lead_str, end_str = "", norm = 'symlog'):
    ...
    bins = 150
    #ret = binned_statistic_2d(x_test[:,0], x_test[:,1], np.abs(errRe*100), statistic=np.mean, bins =[bins, bins])
    H, xedges, yedges, binnumber = binned_statistic_2d(x_test[:N,0], x_test[:N,1], values = vals[:N], statistic=np.mean , bins = [bins, bins])
    #H2, xedges2, yedges2, binnumber2 = stats.binned_statistic_2d(x, y, values = z, statistic='mean' , bins = [20, 20])
    ...
    plt.title("{} {}".format(C_str, title))
    #plt.tight_layout()
    plt.savefig(path + "plots/preds/{}_{}_{}.png".format(lead_str, NAME, end_str), bbox_inches="tight")

def plot_err_feat(path, C_str, NAME, errRe, x_test):
    x_bins = np.linspace(0, 1, 100)
    y_bins = np.linspace(0, 1, 100)
    step = 1/70
    bins = 80
...
def plot_all(path, NAME, mean_err, median_err, eta_1, eta_5, eta_10, history, y_test, y_pred, idcs, errRe, x_test, err):
