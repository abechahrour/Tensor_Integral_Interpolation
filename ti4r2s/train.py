...
def execute(data_path, C_str_feat, C_str, size_str, size_str_test, ext_str ,norm, loss, idx, idx_y,
    ...
    data, y_train = cm.get_data(data_path, C_str_feat, C_str, size_str, ext_str, idx, idx_y, n_train, low, high)
    ...
    #x_test, y_test = get_test_data(path + 'ti4r2s' +'_data/', 'ti4r2s','C010mimi0', size_str_test,ext_str_test, idx)
    print("Count = ", count)
    ...
    plot.plot_grid(C_str, NAME, np.abs(errRe)*100, x_test, '% diff', r'$\frac{t}{s}$', r'$\frac{m^2}{s}$', 'relerrall', '', 'log')
    for i in range(nout):
...
def main(argv):
    ...
    N_func = 4
    func_string=['ti4r2s', 'ti4r2s', 'ti4r2s']
