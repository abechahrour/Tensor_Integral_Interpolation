...
######################################################
    ...
    ext_str = ''
    ext_str_test = ''
    str_data_extra = '_NFT_01'
    extra = '_l{}h{}_{}M{}'.format(low, high, str_ntrain, str_data_extra) ####### ADD UNDERSCORE!!
...
######################################################
    ...
    path_features = path + 'data/'+ '{}_features_{}{}.txt'.format(dir, size_str, str_data_extra)
    NAME = "{}_NFT_nds_{}_lyr_{}_e_{}_b_{}_{}_{}_{}_{}{}".format(dir,nodes, layers, epochs, batch, activation, loss, size_str, norm, extra)

    if real == 0:
        path_labels = path + '/data/'+ '{}_labels_{}_Re{}.txt'.format(dir, size_str, str_data_extra)
        NAME = NAME + "_Re"
    elif real == 1:
        path_labels = path + '/data/'+ '{}_labels_{}_Im{}.txt'.format(dir, size_str, str_data_extra)
        NAME = NAME + "_Im"




    x_test, y_test = cm.get_data_new(path_features, path_labels, n_test, low, high)
    if (not load):
        x_train, y_train = cm.get_data_new(path_features, path_labels, n_train, low, high)
        scaler_data, scaler = cm.scale(norm, path, NAME, low = 0, high = 1e5)
        ...
        print("Scaler Loaded")
    print(NAME)
    print("Shapes of features = ", x_train.shape, x_test.shape)
    print("Shapes of labels = ", y_train.shape, y_test.shape)
