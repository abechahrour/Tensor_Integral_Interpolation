...
######################################################
    ...
    NAME = "{}_disc_nds_{}_lyr_{}_e_{}_b_{}_{}_{}_{}_{}{}".format(dir,nodes, layers, epochs, batch, activation, loss, size_str, norm, extra)
    ...
    x_test, y_test = cm.get_data_new(path_features, path_labels, n_test, low, high, x_high = x_high)
    if (not load):
        x_train, y_train = cm.get_data_new(path_features, path_labels, n_train, low, high, x_high = x_high)
        scaler_data, scaler = cm.scale(norm, path, NAME)
    ...
    if load:
        ...
        model = cm.init_disconnected_model(ndims, nout, nodes, layers, activation, loss)
